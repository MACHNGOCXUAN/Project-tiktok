...
const MENU_ITEM = [
  {
    ...
    herf: "https://www.tiktok.com/live/creators/vi-VN/?enter_from=more&lang=en&region=VN",
  },
  {
    icon: <EnglishIcon />,
    title: "English",
  ...
  return (
    <header className={clsx(Style.wrapper)}>
      <div className={clsx(Style.inner)}>
        <Link to={routerConfig.home} className={clsx(Style.logo)}>
          <svg
        ...
        <div className={clsx(Style.action)}>
          <Button upload icon={<AddIcon />}>
            Upload
          </Button>
          {currentUser ? (
            <>
              <Tippy content="Message" placement="bottom">
                <button className={clsx(Style.message)}>
                  {/* <FontAwesomeIcon icon={faPaperPlane} /> */}
                  <MessageIcon />
                </button>
              </Tippy>
              <Tippy content="Inbox" placement="bottom">
                <button className={clsx(Style.inbox)}>
          ...
          <Menu
            items={currentUser ? MENU_USER : MENU_ITEM}
          ...
          >
            {currentUser ? (
              <Image
                ...
                alt=""
                src="https://p16-sign-sg.tiktokcdn.com/aweme/100x100/tiktok-obj/6977163639924981762.jpeg?lk3s=a5d48078&x-expires=1704546000&x-signature=KLMXe%2BhXupfRpOPmeLKapGFWqjw%3D"
              />
            ) : (
              <button className={clsx(Style.menu)}>
